...
- [x] **[조회]** 사용자는 맛집 리스트를 조회할 수 있다.
    - 모든 맛집 정보를 리스트 형태로 반환한다.
    - 이름, 주소, 한줄 소개 정보가 표시된다.
- [ ] **[조회]** 사용자는 특정 맛집의 상세 정보를 확인할 수 있다.
    - 맛집 ID로 요청을 보내면 상세 정보를 반환한다.
    - 이름, 주소, 한줄 소개, 이용 가능한 스케줄을 확인할 수 있다.
    - **[404]** 맛집 ID를 찾을 수 없는 경우

### 맛집 예약하기

- [ ] **[생성]** 사용자는 특정 맛집의 예약을 생성할 수 있다.
    - 맛집 ID와 스케줄 ID로 대상 리소스를 지정할 수 있다.
    - 예약자 정보(닉네임, 이메일, 예약 비밀번호)를 입력한다.
    - 예약 상세 정보(예약 인원, 예약 메시지)를 입력한다.
    - 예약에 성공하면 확인 메일을 발송한다.
    - **[400]** 요청 정보의 형식이 올바르지 않은 경우
    - **[404]** 맛집 ID, 스케줄 ID를 찾을 수 없는 경우
